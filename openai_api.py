...
client = openai.OpenAI(api_key=DEFAULT_KEY, http_client=http_client, timeout=10)
...
def check_limit(api_key):
    try:
        chatgpt(prompt="Hello!", model="text-ada-001", api_key=api_key, retry=1)
...
def check_api_keys(api_keys):
    for idx, key in enumerate(tqdm(api_keys, dynamic_ncols=True, colour="green", desc="Checking API keys")):
        if check_limit(key):
            print("Good:\t", idx, key)
...
def get_embedding(
    ...
    assert model in OPENAI_EMBEDDING_MODELS, f"model {model} not in {OPENAI_EMBEDDING_MODELS}"
...
def chatgpt_tokenize(text):
    ...
    if chatgpt_tok is None:
        ...
        chatgpt_tok = tiktoken.encoding_for_model("gpt-4o")
...
if __name__ == "__main__":
    # python tool/openai_api.py
    # check_api_keys(api_keys=APIKEYS)
    print(chatgpt(prompt="what is your version?", model="gpt-4o"))
    embedding = get_embedding("Your text goes here 123", model="text-embedding-3-small")
